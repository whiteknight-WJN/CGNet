...
class UNet_PARTX3_2(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(UNet_PARTX3_2, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear

        self.inc = HIBlock(self.n_channels, 64,0)
        self.sgin = SG_IN(2,256)
        ...
        self.up1 = ResUp(1024+512, (512) // factor, bilinear)
        self.skip_DRDB1 = DRDB(512+256, 3, 64, 3)
    ...
    def forward(self, x):
        ...
        x1 = self.inc(g)
        xsg1 = self.sgin(rb)
        xsg1 = F.interpolate(xsg1,scale_factor=0.125)

        x2 = self.down1(x1)
        x3 = self.down2(x2)
        x4 = self.down3(x3)
        x4 = torch.cat([x4, xsg1], 1)
        x5 = self.down4(x4)
        xsg5 = self.sgdown4(xsg1)
        x5 = torch.cat([x5, xsg5], 1)
...
class UNet_PARTX3_3(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(UNet_PARTX3_3, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear

        self.inc = HIBlock(self.n_channels, 64,0)
        self.sgin = SG_IN(2,128)
        self.down1 = HIDown(64, 128)
        self.down2 = HIDown(128, 256)
        self.down3 = HIDown(256+128, 512)
        self.sgdown3 = SG_DOWN(128, 256)
        ...
        self.up1 = ResUp(1024+512, (512+256) // factor, bilinear)
        self.skip_DRDB1 = DRDB(512+256, 3, 64, 3)
        self.up2 = ResUp(512+256, (256) // factor, bilinear)
        self.skip_DRDB2 = DRDB(256+128 , 3, 32, 3)
        self.up3 = ResUp(256, (128) // factor, bilinear)
        self.skip_DRDB3 = DRDB(128, 3, 16, 3)
        self.up4 = ResUp(128, 64, bilinear)
    ...
    def forward(self, x):
        ...
        x3 = self.down2(x2)
        xsg1=F.interpolate(xsg1,scale_factor=0.25)
        x3 = torch.cat([x3, xsg1], 1)
        x4 = self.down3(x3)
        xsg4 = self.sgdown3(xsg1)
        x4 = torch.cat([x4, xsg4], 1)
        x5 = self.down4(x4)
        xsg5 = self.sgdown4(xsg4)
        x5 = torch.cat([x5, xsg5], 1)
...
class UNet_PARTX3_4(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(UNet_PARTX3_4, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear

        self.inc = HIBlock(self.n_channels, 64,0)
        self.sgin = SG_IN(2,64)
        self.down1 = HIDown(64, 128)
        self.down2 = HIDown(128+64, 256)
        self.sgdown2 = SG_DOWN(64, 128)
        self.down3 = HIDown(256+128, 512)
        ...
        self.up2 = ResUp(512+256, (256+128) // factor, bilinear)
        self.skip_DRDB2 = DRDB(256 + 128, 3, 32, 3)
        self.up3 = ResUp(256+128, (128) // factor, bilinear)
        self.skip_DRDB3 = DRDB(128+64, 3, 16, 3)
        self.up4 = ResUp(128, 64, bilinear)
        self.skip_DRDB4 = DRDB(64, 3, 8, 3)
        self.outc = OutConv(64, n_classes)
    ...
    def forward(self, x):
        ...
        x1 = self.inc(g)
        xsg1 = self.sgin(rb)
        xsg1 = F.interpolate(xsg1,scale_factor=0.5)

        x2 = self.down1(x1)
        x2=torch.cat([x2,xsg1],1)
        x3 = self.down2(x2)
        xsg3 = self.sgdown2(xsg1)
        x3 = torch.cat([x3, xsg3], 1)
        x4 = self.down3(x3)
        xsg4 = self.sgdown3(xsg3)
        x4 = torch.cat([x4, xsg4], 1)
...
class UNet_PARTX3_5(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(UNet_PARTX3_5, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear

        self.inc = HIBlock(self.n_channels, 64,0)
        self.sgin0 = SG_IN(2, 32)
        self.sgin = SG_IN(32,32)
        self.down1 = HIDown(64+32, 128)
        self.sgdown1 = SG_DOWN(32, 64)
        self.down2 = HIDown(128+64, 256)
        self.sgdown2 = SG_DOWN(64, 128)
