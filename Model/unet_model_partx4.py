...
class GNBlock(nn.Module):
    def __init__(self, in_size, out_size, relu_slope):
        ...
        self.conv_2 = nn.Conv2d(out_size, out_size, kernel_size=3, padding=1, bias=True)
        self.relu_2 = nn.LeakyReLU(relu_slope, inplace=False)
        self.norm = nn.GroupNorm(out_size,out_size)
...
class LNBlock(nn.Module):
    def __init__(self, in_size, out_size, relu_slope,size):
...
class NODown(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2),
            NOBlock(in_channels, out_channels,0)
        )
    def forward(self, x):
        return self.maxpool_conv(x)
...
class GNDown(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2),
            GNBlock(in_channels, out_channels,0)
        )
    def forward(self, x):
        return self.maxpool_conv(x)


class LNDown(nn.Module):
    """Downscaling with maxpool then double conv"""
    ...
    def __init__(self, in_channels, out_channels,size):
        ...
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2),
            LNBlock(in_channels, out_channels,0,size)
        )
    def forward(self, x):
        return self.maxpool_conv(x)
...
class UNet_PARTX4_1(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(UNet_PARTX4_1, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear

        self.inc = NOBlock(self.n_channels, 64,0)
        self.sgin = SG_IN(2,32)
        self.down1 = NODown(64+32, 128)
        self.sgdown1 = SG_DOWN(32, 64)
        self.down2 = NODown(128+64, 256)
        self.sgdown2 = SG_DOWN(64, 128)
        self.down3 = NODown(256+128, 512)
        self.sgdown3 = SG_DOWN(128, 256)
        factor = 2 if bilinear else 1
        self.down4 = NODown(512+256, 1024 // factor)
        self.sgdown4 = SG_DOWN(256, 512// factor)
...
class UNet_PARTX4_2(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(UNet_PARTX4_2, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear

        self.inc = BABlock(self.n_channels, 64,0)
        self.sgin = SG_IN(2,32)
        self.down1 = BADown(64+32, 128)
        self.sgdown1 = SG_DOWN(32, 64)
        self.down2 = BADown(128+64, 256)
        self.sgdown2 = SG_DOWN(64, 128)
        self.down3 = BADown(256+128, 512)
        self.sgdown3 = SG_DOWN(128, 256)
        factor = 2 if bilinear else 1
        self.down4 = BADown(512+256, 1024 // factor)
        self.sgdown4 = SG_DOWN(256, 512// factor)
...
class UNet_PARTX4_3(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(UNet_PARTX4_3, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear

        self.inc = INBlock(self.n_channels, 64,0)
        self.sgin = SG_IN(2,32)
        self.down1 = INDown(64+32, 128)
        self.sgdown1 = SG_DOWN(32, 64)
        self.down2 = INDown(128+64, 256)
        self.sgdown2 = SG_DOWN(64, 128)
        self.down3 = INDown(256+128, 512)
        self.sgdown3 = SG_DOWN(128, 256)
        factor = 2 if bilinear else 1
        self.down4 = INDown(512+256, 1024 // factor)
        self.sgdown4 = SG_DOWN(256, 512// factor)
...
class UNet_PARTX4_4(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(UNet_PARTX4_4, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear

        self.inc = GNBlock(self.n_channels, 64,0)
        self.sgin = SG_IN(2,32)
        self.down1 = GNDown(64+32, 128)
        self.sgdown1 = SG_DOWN(32, 64)
        self.down2 = GNDown(128+64, 256)
        self.sgdown2 = SG_DOWN(64, 128)
        self.down3 = GNDown(256+128, 512)
        self.sgdown3 = SG_DOWN(128, 256)
        factor = 2 if bilinear else 1
        self.down4 = GNDown(512+256, 1024 // factor)
        self.sgdown4 = SG_DOWN(256, 512// factor)
...
class UNet_PARTX4_5(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(UNet_PARTX4_5, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear
        size=720

        self.inc = LNBlock(self.n_channels, 64,0,(size,size))
        self.sgin = SG_IN(2,32)
        self.down1 = LNDown(64+32, 128,(size//2,size//2))
        self.sgdown1 = SG_DOWN(32, 64)
        self.down2 = LNDown(128+64, 256,(size//4,size//4))
        self.sgdown2 = SG_DOWN(64, 128)
        self.down3 = LNDown(256+128, 512,(size//8,size//8))
        self.sgdown3 = SG_DOWN(128, 256)
        factor = 2 if bilinear else 1
        self.down4 = LNDown(512+256, 1024 // factor,(size//16,size//16))
        self.sgdown4 = SG_DOWN(256, 512// factor)
