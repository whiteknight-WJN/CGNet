...
class UNet_PARTX5_1(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(UNet_PARTX5_1, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear
        ...
        self.inc = HIBlock(self.n_channels, 64, 0)
        factor = 2 if bilinear else 1
        self.down1 = HIDown(64, 128//factor)


        self.skip_DRDB3 = DRDB(64, 3, 16, 3)
...
class UNet_PARTX5_2(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(UNet_PARTX5_2, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear
        ...
        self.sgin = SG_IN(2, 32)
        factor = 2 if bilinear else 1
        self.down1 = HIDown(64 + 32, 128//factor)
        self.sgdown1 = SG_DOWN(32, 64//factor)


        self.skip_DRDB3 = DRDB(64 + 32, 3, 16, 3)
        self.up4 = ResUp(128 + 64, 64 + 32, bilinear)
        self.skip_DRDB4 = DRDB(64 + 32, 3, 8, 3)
        self.outc = OutConv(64 + 32, n_classes)
        self.tanh = nn.Tanh()
...
class UNet_PARTX5_3(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(UNet_PARTX5_3, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear
        ...
        self.sgdown1 = SG_DOWN(32, 64)
        factor = 2 if bilinear else 1
        self.down2 = HIDown(128+64, 256//factor)
        self.sgdown2 = SG_DOWN(64, 128//factor)



        self.skip_DRDB2 = DRDB(128 + 64, 3, 32, 3)
        self.up3 = ResUp(256+128, (128+64) // factor, bilinear)
        self.skip_DRDB3 = DRDB(128+64, 3, 16, 3)
        self.up4 = ResUp(128+64, 64+32, bilinear)
        self.skip_DRDB4 = DRDB(64+32, 3, 8, 3)
...
class UNet_PARTX5_4(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(UNet_PARTX5_4, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear
        ...
        self.sgdown2 = SG_DOWN(64, 128)
        factor = 2 if bilinear else 1
        self.down3 = HIDown(256+128, 512//factor)
        self.sgdown3 = SG_DOWN(128, 256//factor)

        self.skip_DRDB1 = DRDB(256+128, 3, 64, 3)
        self.up2 = ResUp(512+256, (256+128) // factor, bilinear)
        self.skip_DRDB2 = DRDB(256 + 128, 3, 32, 3)
        self.up3 = ResUp(256+128, (128+64) // factor, bilinear)
        self.skip_DRDB3 = DRDB(128+64, 3, 16, 3)
...
class UNet_PARTX5_5(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(UNet_PARTX5_5, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.bilinear = bilinear
        ...
        self.down3 = HIDown(256+128, 512)
        self.sgdown3 = SG_DOWN(128, 256)
        self.down4 = HIDown(512 + 256, 1024)
        self.sgdown4 = SG_DOWN(256, 512)
        factor = 2 if bilinear else 1
        self.down5 = HIDown(1024 + 512, 2048 // factor)
        self.sgdown5 = SG_DOWN(512, 1024 // factor)

        self.skip_DRDB0 = DRDB(1024 + 512, 3, 64, 3)
        self.up0 = ResUp(2048 + 1024,(1024 + 512) // factor, bilinear)
        self.skip_DRDB=DRDB(1024 + 512,3,64,3)
        self.up1 = ResUp(1024+512, (512+256) // factor, bilinear)
        self.skip_DRDB1 = DRDB(512+256, 3, 64, 3)
        self.up2 = ResUp(512+256, (256+128) // factor, bilinear)
        self.skip_DRDB2 = DRDB(256 + 128, 3, 32, 3)
    ...
    def forward(self, x):
        ...
        x6 = self.down5(x5)
        xsg6 = self.sgdown5(xsg5)
        x6 = torch.cat([x6, xsg6], 1)

        x6=self.skip_DRDB0(x6)
        x5=self.skip_DRDB(x5)
        x4 = self.skip_DRDB1(x4)
        x3 = self.skip_DRDB2(x3)
        x2 = self.skip_DRDB3(x2)
