...
output_dir = r'./result_origin_raw_mix'
if not os.path.exists(output_dir):
    os.makedirs(output_dir)
device=torch.device('cuda:4')
G = Generator().to(device)
G_static=torch.load('./checkpoints/aia2ha_new_unet_origin/Model/height_512/460_G.pt')
...
for i1,r in zip(list1,rata):
    ...
    print('PSNR=%.4f,SSIM=%.4f'%(np.mean(psnr_value),np.mean(ssim_value)))
