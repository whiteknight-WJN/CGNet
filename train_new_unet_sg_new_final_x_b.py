...
if __name__ == '__main__':
    ...
    from model_new_unet_sg_new_final import Generator_X1,Generator_X2,Generator_X3,Loss,Generator_BA,Generator_IN
    ...
    os.environ['CUDA_VISION_DEVICES'] = '0,1,3,6'
    ...
    G1 = Generator_IN().to(DEVICE)
    G_static = torch.load('./checkpoints/aia2ha_new_unet_new_sg_final_in/Model/height_512/300_G.pt')
    model_static = {k.replace('module.', ''): v for k, v in G_static.items()}
    # model_static.update(G_static)
    G1.load_state_dict(model_static)


    G=nn.DataParallel(G1,device_ids=[0,1,3,6])
