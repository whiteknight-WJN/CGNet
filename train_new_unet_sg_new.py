if __name__ == '__main__':
    ...
    from model_new_unet_sg_new import Generator,Loss
    ...
    G_static = torch.load('./checkpoints/aia2ha_new_unet_new_sg_all/Model/height_512/400_G.pt')
    model_static = {k.replace('module.', ''): v for k, v in G_static.items()}
    # model_static.update(G_static)
    G1.load_state_dict(model_static)

    G = nn.DataParallel(G1, device_ids=[2, 3, 4, 6, 7])
